...
function showsub(id){
  var submenu=document.getElementById("second"+id);
...
function stopturn(){
	clearInterval(u_u36_icn.currentTimer)
};
function startturn(){
    u_u36_icn.currentTimer=window.setInterval(u_u36_icn.onchangeimg(),500)
};
function ImageChangeNews(uid, width, height, interval, duration, btitle, bsummary, zoomtype, zoomout)
{
    ...
    {       
        ...
        _this.currentTimer = window.setInterval(_this.onchangeimg, interval * 500);//设置循环周期为4000
